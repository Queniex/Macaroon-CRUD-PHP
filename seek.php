...
<body>
    ...
    <div class="bg-[#FFFBEB]">
        ...
        <div class="isi h-[70vh]">
            <div class="container mb-2 flex justify-center">

                <div class="card border-solid border-black border-2 mt-8 mb-3 bg-[white]" style="max-width: 30rem;">
                    <div class="border-solid border-black border-b-[1px] bg-[pink] card-header flex justify-center">
                        <h1 class="m-3 modal-title fs-5" id="beli" style="color: rgb(0, 0, 0); text-decoration: underline;text-decoration-color: rgb(255, 87, 213);  ">Data Pelanggan 🛒 || <span>(<?= $item["tgl_transaksi"]; ?>)</span></h1></div>
                    <div class="card-body text-dark -mb-3">
                      <p class="card-text -mt-5">
                        <div>Nama Pelanggan : <span><?= $item["nama"]; ?></span></div>
                        <div>Hp Pelanggan : <span><?= $item["hp"]; ?></span></div>
                        <div>Alamat Pelanggan : <span><?= $item["alamat"]; ?></span></div>
                        <hr>
                        <div>Barang Terbeli : <span><?= $item["nama_barang"]; ?></span></div>
                        <div>Jenis Barang : <span><?= $item["jenis_barang"]; ?></span></div>
                        <div>Harga Barang : <span><?= $item["harga"]; ?></span></div>
                        <div>Jumlah Barang : <span><?= $item["jumlah"]; ?></span></div>
                        <hr>
                        <div>Total Harga : <span style="background-color: #b900b9;"><?= $item["total"]; ?></span></div>
                      </p>
                    </div>
                  </div>
